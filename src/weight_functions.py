...
def view_weight(weight_file_name):
    with open(weight_file_name, "r") as weight_file:
        ...
        for row in reader:
            if (row[1] == "True"):
                print(
                    f"{fg('blue')}Bodyweight:{attr('reset')} {row[0]} {fg('green')}This weeks weight is less then last week!{attr('reset')}")
            else:
                print(
                    f"{fg('blue')}Bodyweight:{attr('reset')} {row[0]} {fg('red')}This weeks weight is more then last weeks!{attr('reset')}")
